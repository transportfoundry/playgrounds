...
public class VehicleGeneration extends MatsimXmlWriter {

	private Scenario scenario;
	private static int counter = 0;
	private static int counterTW = 0;
	private static int counterFF = 1;

	
	public VehicleGeneration(Scenario scenario) {
	...
	public void write(String file) {
		...
		*/
		...
		attsC2.add(new Tuple<>("name", "Catchacar"));
		...
		attsC3.add(new Tuple<>("name", "Smide"));
	...
	private void writeVehicles() {
		...
		for (int i = 0; i < cars; i++) {
			...
			Link link = (Link) array[r.nextInt(numberLinks)];
			writeFFVehicle(link, counter, r);
			counter++;
		}		
	}
	
	
	private void writeBikes() {
		Network network = this.scenario.getNetwork();
		...
		for (int i = 0; i < bikes; i++) {
			...
			Link link = (Link) array[r.nextInt(numberLinks)];
			writeBSVehicle(link, counter, r);
			counter++;
		}		
	}
	

	private void writeFFVehicle(Link link, int id, Random random) {
	...
	private void writeBSVehicle(Link link, int id, Random random) {
		
		List<Tuple<String, String>> attsV = new ArrayList<Tuple<String, String>>();
		
		attsV.add(new Tuple<>("id", "BS_" + Integer.toString(id)));
		attsV.add(new Tuple<>("x", Double.toString(link.getCoord().getX())));
		attsV.add(new Tuple<>("y", Double.toString(link.getCoord().getY())));
		
	//	if (random.nextDouble() < 0.9)
		
			attsV.add(new Tuple<>("type", "bike"));
	//	else
	//		attsV.add(new Tuple<>("type", "transporter"));

		writeStartTag("bikeshare", attsV, true);		
	}
	...
	public static void main(String[] args) {
        ...
        VehicleGeneration place = new VehicleGeneration(scenario);
		place.write(args[1]);		
	}
