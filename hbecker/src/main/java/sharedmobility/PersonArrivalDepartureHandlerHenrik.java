...
public class PersonArrivalDepartureHandlerHenrik implements PersonDepartureEventHandler, PersonLeavesVehicleEventHandler, 
...
public void handleEvent(PersonDepartureEvent event) {
	...
	else if (legMode.equals("egress_walk_bs")) {
		String vehId = personLeavesVehicleMap.get(event.getPersonId()).toString();
		Id<Link> linkId = personArrivalMap.get(event.getPersonId());
		carsharingManager.parkVehicle(vehId, linkId);
		Link link = network.getLinks().get(linkId);
		this.currentDemand.removeVehicle(event.getPersonId(), link, carsharingSupply.getAllVehicles().get(vehId), "bikeshare");
		eventsManager.processEvent(new EndRentalEvent(event.getTime(), linkId, event.getPersonId(), vehId));
...
public void handleEvent(PersonArrivalEvent event) {
	...
	if (mode.startsWith("free") || 
			mode.startsWith("one") || mode.startsWith("two") || mode.startsWith("bikes")) {			
...
public void handleEvent(PersonEntersVehicleEvent event) {
	...
	if (arrivalMode != null) {
		...
		else if (vehId.startsWith("BS") &&
				!arrivalMode.equals("access_walk_bs")) {
			Id<Link> linkId = this.personDepartureMap.get(event.getPersonId());
			Link link = network.getLinks().get(linkId);
